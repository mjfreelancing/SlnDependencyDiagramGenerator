...
The application shall use Serilog as the primary runtime output channel for generator and tool execution diagnostics. The generator and renderers use `ILogger<T>` (from `Microsoft.Extensions.Logging`) so all output is available through the Serilog pipeline. For the first release, runtime output shall be captured through an in-memory `CircularBufferSink` (from `AllOverIt.Serilog`) used by the UI output panel, and also persisted through a rolling file sink for troubleshooting past runs. The `ObservableSink` (also from `AllOverIt.Serilog`) may be used for live ReactiveUI streaming to the output panel. The rolling log file shall be named `{projectFileBaseName}-{Date}.txt` and written to a log directory configured in application settings. Log-file retention shall be user-configurable in application settings with a default value of 30 days. The shared `UseStudioSerilog` extension (in `SlnDependencyStudio.Shared`) accepts the config file base name and log directory from each frontend. The UI output panel shall visually distinguish log levels by colour: Error (red), Warning (yellow), Information (white), Debug (gray). Diagram and image files produced by generation remain the primary generation artifacts.
